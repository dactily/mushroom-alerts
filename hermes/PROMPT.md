...
Вставить в cron-задачу Hermes, время запуска — 08:30 Europe/Prague
(к этому часу обновлены карта ČHMÚ, HoubyMapa и открытые данные станций).
...
Когда отправлять:
- сегодня хотя бы в одной локации вердикт «высокая»; или
- в пределах 7 дней ожидается «высокая» / «хорошие условия»; или
- вердикт по любой локации изменился по сравнению со вчерашним.
Иначе — молчи, ничего не отправляй.

Память: после каждого запуска сохраняй в память одну строку вида
  «грибы YYYY-MM-DD: Valmez=средняя, Bystrice=средняя, ближайшее окно 22.9»
и сравнивай с ней на следующий день, чтобы понять, изменился ли вердикт.
Держи только последнюю такую строку, старую перезаписывай.

По воскресеньям отправляй короткий дайджест всегда, даже если условия
не менялись: вердикт по локациям, что было за неделю, что ждать дальше.
